...
def get_tags_from_file(filepath):
    ...
    try:
        # Use the correct pyexiv2.Image() API
        img = pyexiv2.Image(str(filepath))
        xmp_data = img.read_xmp()
        img.close() # Close the file handle
        # Ensure we return a list, even if the tag is empty or None
        return xmp_data.get('Xmp.dc.subject', []) or []
    except Exception as e:
        # Silently fail on read errors during list/auto-tag
...
def set_tags_on_file(filepath, tags_to_add):
    ...
    """
    try:
        # Use the correct pyexiv2.Image() API to open for read/write
        img = pyexiv2.Image(str(filepath))
        
        # Get existing tags
        xmp_data = img.read_xmp()
        existing_tags = xmp_data.get('Xmp.dc.subject', []) or []
        ...
        # Close the file (this saves the changes)
        img.close()
        ...
        # Fix for UnboundLocalError: return a safe empty list
        return (False, [])
...
def cmd_auto_tag(args):
    ...
    for img_path in image_files:
        ...
        # --- Start with an empty list for all generated tags ---
        generated_tags = []

        # --- 1. Build tags from folder structure ---
        if parts:
        ...
        # --- 2. Build tags from filename (if flag is set) ---
        if args.tags_from_filename:
            ...
            filename_tags = []
            for token in tokens:
                # Filter: Keep tokens longer than 2 chars (now includes numeric)
                if token and len(token) > 2:
                    filename_tags.append(token.lower())
            
            if filename_tags:
                generated_tags.extend(filename_tags)
        
        # --- Continue if no tags were generated by either method ---
        if not generated_tags:
            continue
            
        # Add the unique set of generated tags to our summary
...
if __name__ == "__main__":
    ...
    # Call the function associated with the chosen subcommand (e.g., cmd_auto_tag)
    args.func(args)